...
router.get('/follow/status/:id', authMiddleware, async (req, res) => {
    const currentUserId = req.userId;
    const targetId = Number(req.params.id);
    const prisma = getPrisma();
...
router.get('/profile/:id/likes', async (req, res) => {
    const userId = Number(req.params.id);
    const currentUserId = await getUserId(req);
    if (!userId || isNaN(userId)) {
        return res.status(400).json({ message: 'Invalid user id' });
...
router.get('/profile/:id/saved', async (req, res) => {
    const userId = Number(req.params.id);
    const currentUserId = await getUserId(req);
    if (!userId || isNaN(userId)) {
        return res.status(400).json({ message: 'Invalid user id' });
    ...
    `;
        ...
        const savedWithStatus = await Promise.all(savedPosts.map(async (saved) => {
...
router.delete('/profile/followers/:followerId', authMiddleware, async (req, res) => {
    const currentUserId = req.userId;
...
// Delete own comment
router.delete('/profile/comments/:commentId', authMiddleware, async (req, res) => {
    const currentUserId = req.userId;
